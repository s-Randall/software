...
for ii=[1:30]
    ii
    filename=strcat('F:\资料\小动物\16模块\SourceData\16module_Source_',num2str(ii),'.mat');
    load(filename);
...
%     plot(data(:,3));
    ...
    load('F:\资料\小动物\16模块\SourceData\delay.mat');
    for i=0:15
        index=find(data(:,4)==i);
        data(index,3)= data(index,3)-delay(i+1);
    end
    timeS=data;
...
%     timeF(index,7)=timeF(index,7)+100;
    ...
    index=find(timeF(:,2)>=500&timeF(:,2)<=700&timeF(:,7)>=500&timeF(:,7)<=700);
    timeF=timeF(index,:);
    % timeT=timeF(:,[1 4 6 9]);
