...
if nargin == 2
    channel = 101;
    index = find(single_event_data(:,4) == module);
elseif nargin == 3
    index = find(single_event_data(:,4 )== module & single_event_data(:,1) == channel);
end
if max(index)>0 % 指定数据非空
    %取进行拟合的数据
    ...
    peak
    if ~isempty(peak) % 找到了谱峰
        if peak<150 || peak>800
            peak = 511;
        end
